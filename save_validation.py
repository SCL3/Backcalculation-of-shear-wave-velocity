...
nLayer = 101 # fixed number of real pavement layers, must be within h_bound
numOut = 101 # concat three values of thickness (except inf) with four values of Vs => 7 discrete values
...
class VelocityProfilePlotter:
    ...
    def _create_velocity_subplot(self, fig, out_np, target_np=None):
        ...
        predicted_h = np.append(np.full(100, 0.01), np.inf)
        predicted_Vs = out_np
    ...
    def _add_target_plots(self, ax, target_np, pred_depths):
        ...
        # h = np.full(100, 0.01)
        # h[-1] = np.inf
        h = np.append(np.full(100, 0.01), np.inf)
        Vs = target_np
...
def save_prediction(input_fvs, input_fls, predict, file_id, folder_save_result='validation_results',
    ...
    input_fls_path = os.path.join(folder_save_result, f"{file_id}_input_fls.mat")
    scipy.io.savemat(input_fls_path, {'fls': input_fls})  # data is the fieldname
